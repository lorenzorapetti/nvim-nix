...
--- For the plain `yaml` filetype (not `yaml.docker-compose`, `yaml.gitlab`, etc., which
--- already imply a known schema by convention), only attach if the buffer has a schema
--- associated with it: a modeline/`$schema` directive near the top of the file, or a
--- `schema.json` file somewhere between the buffer and the project root.
local function has_schema_directive(bufnr)
  local lines = vim.api.nvim_buf_get_lines(bufnr, 0, 10, false)
  for _, line in ipairs(lines) do
    if
      line:match('^%s*#%s*yaml%-language%-server:.*%$schema=')
      or line:match('^%s*#%s*%$schema:')
      or line:match('^%s*%$schema:')
    then
      return true
    end
  end
...
local function has_schema_json(bufnr)
  local fname = vim.api.nvim_buf_get_name(bufnr)
  local root = vim.fs.root(bufnr, { '.git' })
  return vim.fs.find('schema.json', {
    path = vim.fs.dirname(fname),
    upward = true,
    type = 'file',
    stop = root and vim.fs.dirname(root),
  })[1] ~= nil
end
...
return {
  ...
  root_dir = function(bufnr, on_dir)
    ...
    if ft == 'yaml' and not (has_schema_directive(bufnr) or has_schema_json(bufnr)) then
      return
    end
